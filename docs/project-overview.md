...
当前稳定可用的是 `html-page` 网页翻译。PDF 和 YouTube 字幕翻译已经接入首版实验链路：

- PDF：独立 PDF 标签页会打开扩展工作台，当前仅支持可提取文本层的 PDF。扫描版 PDF 的 OCR 兜底尚未接入。
- YouTube：`youtube.com/watch` 页面会尝试读取已有字幕轨道并渲染翻译 overlay。无字幕视频的 ASR 兜底尚未接入。
...
- `START_TRANSLATION_JOB`
  - 多目标翻译统一入口。Popup 使用它自动路由普通网页、PDF 与 YouTube 字幕；YouTube watch 页的悬浮球也会使用它触发字幕翻译。
