  ...
  chrome.runtime.onMessage.addListener((
    ...
    if (messageType === 'COLLECT_YOUTUBE_SUBTITLE_SEGMENTS') {
      ...
      void collectYoutubeSubtitleSegments(youtubeMessage.preferredLanguage, {
        enableAutoGeneratedCaptions: youtubeMessage.enableAutoGeneratedCaptions ?? true,
      })
        .then((segments) => {
          if (segments.length > 0) {
            renderYoutubeSubtitleOverlay(
              [],
              youtubeMessage.displayMode,
              youtubeMessage.subtitleDisplayStyle,
              { sendRuntimeMessage: (runtimeMessage) => chrome.runtime.sendMessage(runtimeMessage) },
            );
            reserveTrackCueIds(segments.map((segment) => segment.id));
          }
          sendResponse(segments);
        })
        .catch((error) => {
