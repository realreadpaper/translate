...
const defaultSettings = {
  ...
  youtubeAsrFallback: 'disabled' as const,
...
describe('createMessageHandler', () => {
  ...
  it('translates provided youtube-subtitles segments without collecting the full track again', async () => {
    ...
    expect(translatePage).toHaveBeenCalledWith(
      [{ id: 'rendered-cue-1', text: 'Welcome back' }],
      expect.objectContaining({
        targetLanguage: 'zh-CN',
        contentKind: 'youtube-subtitles',
      }),
  ...
  it('explains that ASR cannot prefetch YouTube audio when subtitle sources are unavailable', async () => {
    ...
    const handler = createMessageHandler({
      ...
      loadSettings: vi.fn().mockResolvedValue({
        ...defaultSettings,
        youtubeAsrFallback: 'realtime',
      }),
      detectTarget: vi.fn().mockResolvedValue(target),
      openPdfWorkspace: vi.fn(),
    });

    await expect(handler({ type: 'START_TRANSLATION_JOB', tabId: 7 })).rejects.toThrow(
      '实时 ASR 需要视频播放后才能采集标签页音频',
    );
  });

  it('describes sync-delay ASR as an explicit playback-delay fallback when collection fails', async () => {
    const sendMessageToTab = vi.fn().mockResolvedValue({
      ok: false,
      message: '当前视频没有可用字幕轨道。',
    });
    const target = {
      kind: 'youtube-subtitles',
      tabId: 7,
